...
// ==========================================
// 7. PHASE MATERIAL REQUESTS LIST MODAL
// ==========================================
interface PhaseMaterialRequestsListModalProps {
  isOpen: boolean;
  onClose: () => void;
  phase: WBSPhase;
  allMaterialRequests: import('../services/projectService').MaterialRequest[];
...
export const PhaseMaterialRequestsListModal: React.FC<PhaseMaterialRequestsListModalProps> = ({
  isOpen,
  onClose,
  phase,
  allMaterialRequests,
  ...
  const phaseRequests = allMaterialRequests.filter(r => r.phaseId === phase.id);

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending_leader':
        return <span className="badge badge-warning">Chờ Leader</span>;
      case 'approved_by_leader':
        return <span className="badge badge-info">Đã tổng hợp</span>;
      case 'pending_accountant':
        return <span className="badge badge-warning">Chờ Kế toán</span>;
      case 'pending_disbursement':
        return <span className="badge badge-warning">Chờ Tạm ứng</span>;
      case 'pending_director':
        return <span className="badge badge-warning">Chờ Giám đốc</span>;
      case 'approved':
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Danh sách Yêu cầu Vật tư: ${phase.name}`}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '70vh', overflowY: 'auto', paddingRight: '4px' }}>
        
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <button onClick={onCreateNew} className="btn btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Plus size={16} /> Tạo Yêu cầu Vật tư
          </button>
        </div>

        {phaseRequests.length === 0 ? (
          <div style={{ padding: '20px', textAlign: 'center', color: 'hsl(var(--text-muted))', backgroundColor: 'hsl(var(--bg-main))', borderRadius: 'var(--radius-sm)' }}>
            Chưa có yêu cầu vật tư nào cho giai đoạn này.
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {phaseRequests.map(r => (
              <div key={r.id} style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '12px', backgroundColor: 'hsl(var(--bg-card))', borderRadius: 'var(--radius-sm)', border: '1px solid hsl(var(--border))' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ fontWeight: 600, color: 'hsl(var(--text-primary))' }}>{r.requesterName}</div>
                    <div style={{ fontSize: '0.8rem', color: 'hsl(var(--text-muted))' }}>{r.date}</div>
                    {r.taskName && (
                      <div style={{ fontSize: '0.85rem', color: 'hsl(var(--text-secondary))', marginTop: '4px' }}>
                        Task: <strong>{r.taskName}</strong>
                      </div>
                    )}
                  </div>
                  <div>
                ...
                <div style={{ marginTop: '8px', padding: '8px', backgroundColor: 'hsl(var(--bg-main))', borderRadius: 'var(--radius-sm)' }}>
                  <div style={{ fontSize: '0.8rem', fontWeight: 600, marginBottom: '4px', color: 'hsl(var(--text-secondary))' }}>Chi tiết vật tư:</div>
                ...
                {r.reason && (
                  <div style={{ fontSize: '0.85rem', color: 'hsl(var(--text-secondary))', fontStyle: 'italic', marginTop: '4px' }}>
                    Ghi chú: {r.reason}
                  </div>
                )}
              </div>
