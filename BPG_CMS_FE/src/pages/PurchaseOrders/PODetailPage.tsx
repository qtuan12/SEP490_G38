...
// Lịch sử phiếu cần biết chính xác thời điểm (không chỉ ngày) vì nhiều thao tác có thể xảy ra
// trong cùng một ngày. Backend trả DateTime không kèm 'Z' (SQL Server đánh mất Kind=Utc khi đọc
// lại), nên phải dùng formatDateVietnam (quy đổi đúng giờ VN) — new Date() trần sẽ coi nhầm giờ
// UTC là giờ máy người xem, lệch mất 7 tiếng.
const fmtDateTime = (s?: string | null) => (s ? formatDateVietnam(s) : '—');
...
export const PODetailPage: React.FC = () => {
  ...
  // Phiếu tự sinh từ mua khẩn cấp (xem DirectPurchaseFulfillmentService.MaterializeAsync)
  // bỏ qua toàn bộ quy trình trình/duyệt của Giám đốc, nên "Người trình"/"Người duyệt" vô nghĩa.
  const isAutoGeneratedDP = po.poNumber.startsWith('DP-PO-');

  // Lịch sử phiếu: gộp ai-làm-gì-khi-nào theo đúng trình tự nghiệp vụ (lập → duyệt/từ chối →
  // hủy/đóng), thay vì rải rác ở nhiều banner phía trên như trước.
  type HistoryEvent = { key: string; icon: React.ReactNode; color: string; bg: string; label: string; date?: string; note?: string };
  const historyEvents: HistoryEvent[] = [];
  if (!isAutoGeneratedDP) {
    if (po.creatorName) {
      historyEvents.push({ key: 'created', icon: <User size={14} />, color: 'hsl(var(--text-secondary))', bg: 'hsl(var(--bg-hover))', label: `${po.creatorName} lập đơn hàng`, date: po.createdAt });
    }
    if (po.status === 'Rejected') {
      historyEvents.push({ key: 'rejected', icon: <XCircle size={14} />, color: 'hsl(var(--danger))', bg: 'hsl(var(--danger-glow))', label: po.approverName ? `${po.approverName} từ chối đơn hàng` : 'Đơn hàng bị từ chối', date: po.approvedAt, note: po.rejectedReason });
    } else if (po.approverName) {
      historyEvents.push({ key: 'approved', icon: <CheckCircle2 size={14} />, color: 'hsl(142 70% 40%)', bg: 'hsl(142 70% 45% / 0.12)', label: `${po.approverName} duyệt đơn hàng`, date: po.approvedAt, note: po.approvalNote });
    }
  }
  if (po.cancelledByName) {
    historyEvents.push({ key: 'cancelled', icon: <Ban size={14} />, color: 'hsl(var(--danger))', bg: 'hsl(var(--danger-glow))', label: `${po.cancelledByName} hủy đơn hàng`, date: po.cancelledAt, note: po.cancelledReason });
  }
  if (po.closedByName) {
    historyEvents.push({ key: 'closed', icon: <Lock size={14} />, color: 'hsl(var(--primary))', bg: 'hsl(var(--primary-glow))', label: `${po.closedByName} đóng đơn hàng`, date: po.closedAt, note: po.closedReason });
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 1120, margin: '0 auto' }}>
      ...
      {/* Rejected reason banner — ai từ chối/khi nào xem ở khối "Lịch sử phiếu" bên dưới */}
      {po.status === 'Rejected' && (
        <div style={{
          display: 'flex', alignItems: 'flex-start', gap: 10,
      ...
      {/* Closed reason banner — ai đóng/khi nào xem ở khối "Lịch sử phiếu" bên dưới */}
      ...
      {/* Lịch sử phiếu: ai lập / duyệt / từ chối / hủy / đóng và khi nào, theo trình tự nghiệp vụ.
          Đặt cuối trang vì đây là thông tin tra cứu thêm, không phải nội dung chính của đơn hàng. */}
        ...
        <div className="glass-panel p-6">
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {historyEvents.map((ev, idx) => (
              <div key={ev.key} style={{ display: 'flex', gap: 12, paddingBottom: idx < historyEvents.length - 1 ? 14 : 0 }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                  ...
                  {idx < historyEvents.length - 1 && (
                    <span style={{ width: 1, flex: 1, background: 'hsl(var(--border))', marginTop: 4 }} />
                  )}
                </div>
                <div style={{ paddingTop: 3 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: 'hsl(var(--text-primary))' }}>{ev.label}</span>
                    {ev.date && (
                      <span style={{ fontSize: 12, color: 'hsl(var(--text-muted))' }}>{fmtDateTime(ev.date)}</span>
                    )}
                  </div>
                  {ev.note && (
                    <div style={{ fontSize: 13, color: 'hsl(var(--text-secondary))', marginTop: 2 }}>{ev.note}</div>
                  )}
                </div>
              </div>
            ))}
