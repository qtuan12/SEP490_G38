...
import {
  ...
  CreditCard, FileText, Package, Link2, AlertCircle, Loader2, XCircle, Ban,
} from 'lucide-react';
...
export const PODetailPage: React.FC = () => {
  ...
  const cancelMutation = useMutation({
    ...
    onError: (err: any) => setCancelError(err.message || 'Hủy PO thất bại.'),
  });
  ...
  if (isLoading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 300, gap: 10 }}>
        ...
        <span style={{ color: 'hsl(var(--text-secondary))' }}>Đang tải chi tiết PO...</span>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 1120, margin: '0 auto' }}>
      ...
      <div className="glass-panel p-6" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '20px 32px' }}>
        <InfoItem icon={<CalendarDays size={14} />} label="Ngày PO" value={fmtDate(po.orderDate)} />
        ...
        <InfoItem icon={<CreditCard size={14} />} label="Điều khoản thanh toán" value={po.paymentTerms || undefined} />
        {po.notes && (
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {po.linkedRequests.map((req) => (
              <div key={req.requestId} style={{
                ...
                <span style={{ fontWeight: 600 }}>#{req.requestId}</span>
