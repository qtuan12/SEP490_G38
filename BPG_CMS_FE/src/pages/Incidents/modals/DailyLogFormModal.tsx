...
export const DailyLogFormModal: React.FC<DailyLogFormModalProps> = ({
  ...
  // Initialize form and states when modal opens or when currentTask is loaded/changed
  useEffect(() => {
    if (isOpen) {
      setSelectedFiles([]);
      setPreviews([]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEditMode ? "Sửa Nhật ký công trường" : "Cập nhật Nhật ký công trường"}>
