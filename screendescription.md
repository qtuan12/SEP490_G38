...
- Logout clears login session data, resets query cache, and redirects to login page.
...
**Data processing**
- Loads project detail data via backend project services.
...
**Data processing**
- Calls aggregate services for director stats and global risk alerts.
...
**Data processing**
- Fetches technical metrics via backend services.
...
**Data processing**
- Validates form input fields and posts payload to project backend services.
...
**Abnormal cases**
- Missing project returns Not Found. Service errors display an error state. Unauthorized operations are hidden and independently rejected by the backend authorization pipeline. Empty WBS shows guidance to create the first phase.
...
**Data processing**
- Queries project member list via project member services.
- Updates member role assignments via leader designation and member removal services.
...
**Data processing**
- Fetches all available system technical users via user management service authorized for Admin and Technical Manager.
- Filters user list locally based on search input text.
- Submits selected user IDs payload to project member assignment service.
...
**Data processing**
- Fetches project attachments list via project drawing service services.
...
### 3.11.8. Project Daily Logs Screen

**Function type:** Screen (embedded project tab)

**Function trigger:** Projects → select a project → Daily Logs tab.

**Function description:** Timeline feed screen reviewing daily construction updates, progress changes (+%), evidence photos, and comments. Supports SignalR real-time synchronization. Actors include Site Engineer, PL, Technical Manager, Admin.

**Function Details**

**Interface and data**
- Filter controls: Keyword search, Phase select, Subtask select, Assigned Engineer select, Date Range pickers.
- Action button "Tạo nhật ký mới" to open daily log form modal.
- Drawer button to open Progress History timeline for a selected task.
- Vertical feed of daily log cards displaying author, timestamp, progress delta (+%), description, photos, and comment section.

**Data processing**
- Queries daily log services with active filter parameters.
- Listens to SignalR `DailyLogCreated` and `CommentAdded` events to update feed live.

**Validation and business rules**
- Engineers can edit/delete only their own daily log entries within 24 hours of creation.

**Normal cases**
- User filters feed, views site photos in lightbox modal, writes comments, or opens progress history drawer.

**Abnormal cases**
- Empty log feed renders "No daily logs reported" placeholder.

---

### 3.11.9. Daily Log Form Modal

**Function type:** Modal dialog

**Function trigger:** Click "Báo cáo tiến độ ngày" on task row in WBS workspace, Gantt chart, or Daily Logs screen.

**Function description:** Modal form allowing assigned Site Engineers or PLs to report daily completion progress %, write work descriptions, and upload site photos. Actors include Site Engineer, Project Leader.

**Function Details**

**Interface and data**
- Mandatory Task dropdown (pre-filled if opened from task row).
- Mandatory New Progress % number input (0 - 100%).
- Mandatory Log Description textarea (min 5 characters).
- Drag-and-drop file uploader for up to 5 site photos (max 10MB each).

**Data processing**
- Submits form payload to daily log backend service.
- Uploads image files to file storage service and attaches media URLs to log record.

**Validation and business rules**
- Progress % must be between 0 and 100%.
- Decreasing progress % requires mandatory justification notes in description.
- File upload accepts only JPG, PNG, WEBP formats under 10MB.

**Normal cases**
- Engineer inputs updated progress %, attaches site photos, and submits log entry.

**Abnormal cases**
- Uploading non-image file triggers file validation error alert.
...
**Data processing**
- Queries inventory adjustments service filtered by projectId, type, and status.
...
**Data processing**
- Posts payload to inventory adjustment increase service.
...
- Posts payload to inventory adjustment decrease service.
...
**Data processing**
- Calls approval or rejection services for adjustment slips.
...
- Posts payload to incidents service with status WaitingAccountant.
...
**Data processing**
- Calls incident confirmation or rejection services.
...
**Data processing**
- Posts payload to incidents service with isEmergency = true.
...
- Upon approval: calls incident resolution service, executes proposed rework task creation or progress reduction, and sets incident status to Approved.
- Upon rejection: calls incident rejection service with mandatory rejection reason text, updates incident status to Rejected, and notifies the reporter.
...
**Function type:** Screen (standalone report workspace)

**Function trigger:** Sidebar -> Reports menu.

**Function description:** Central reporting workspace for management users. The backend controller is protected by `RolePolicies.Reports`, so only Admin, Director, Technical Manager, and Accountant can access report services.

**Function Details**

**Interface and data**
- Project selector supports a specific project or `projectId = 0` for all accessible projects.
- Global filters pass `fromDate` and `toDate` to report services that support period filtering.
- Available report views are:
...
**Data processing**
- Every exposed Reports service validates the Reports role at controller level.
- Report handlers validate project access through `IProjectAccessService` where project-scoped data is queried.
- Admin, Director, Technical Manager, and Accountant can access all projects through project access rules.
- `projectId = 0` aggregates only projects accessible to the current user.

**Validation and business rules**
- Site Engineer and Project Leader are not part of the Reports role policy for the Reports Hub services.
- Invalid or inaccessible project selection returns a business forbidden error.
- Date range values are normalized to full days by handlers that accept period filters.

**Normal cases**
- Authorized user selects a project scope, applies date filters, and switches between report views.

**Abnormal cases**
- Unauthorized users are blocked by authorization middleware.
- Empty data returns zero-valued cards and empty tables instead of failing.

---

### 3.14.2. Executive Dashboard Report Tab

**Function type:** Screen (embedded report tab)

**Function trigger:** Reports Hub -> "Tổng quan" tab.

**Function description:** Displays executive KPIs for one project or all accessible projects.

**Function Details**

**Interface and data**
- Metric cards: Total Tasks, Completed Tasks, In Progress Tasks, Delayed Tasks, At Risk Tasks, Materials Exceeding BOQ, Over-BOQ Material Requests.
- Phase breakdown table: Phase Name, Status, Total Tasks, Completed Tasks, Progress %.
- Delayed/at-risk task list: Task Name, Phase, Progress %, End Date, Assignee, Warning Type.
- Period comparison block: current/previous completed tasks, incidents, procurement cost, and over-BOQ request counts.
- Cross-project matrix: Project Name, Status, Progress %, delayed count, over-BOQ count, incident count, estimated loss, health status.
- Monthly progress trend: planned cumulative progress, actual cumulative progress, planned monthly volume, actual monthly volume.

**Data processing**
- Excludes obsolete tasks from active totals.
- Task scope applies date overlap filtering when a date range is selected.
- BOQ material count is distinct by material ID from over-BOQ request items.
- Monthly actual progress uses `TaskProgressLog` snapshots at each month end where available.

**Validation and business rules**
- Delayed = due date has passed and task is not completed.
- At Risk = task has started, is not completed, has 3 days or fewer left, and is at least 20% behind expected progress.
- Health status in cross-project matrix is calculated from delayed tasks, estimated incident loss, and over-BOQ request count.

**Normal cases**
- User reviews project health and drills into phase/task warnings.

**Abnormal cases**
- Project without tasks displays zero task metrics and empty phase/task lists.

---

### 3.14.3. Consolidated Executive Report Tab

**Function type:** Screen (embedded report tab / export-ready summary)

**Function trigger:** Reports Hub -> consolidated executive report action.

**Function description:** Presents a combined management report generated from executive dashboard, construction progress, BOQ, incident, and procurement reports.

**Function Details**

**Interface and data**
- Header: Project Name, GeneratedAt, FromDate, ToDate.
- Sections: Executive Metrics, Progress Summary, BOQ Summary, Incident Summary, Procurement Summary, Cross-Project Matrix.
- Executive Insights list generated from child report data.

**Data processing**
- Backend calls child report handlers sequentially to avoid concurrent EF Core operations on the same scoped DbContext.
- If project ID is greater than 0, project name is loaded from the Projects table; otherwise aggregate label is used.

**Validation and business rules**
- Any missing child report DTO returns `ERR_REPORT_DATA_EMPTY`.
- The report is read-only and does not persist generated insight text.

**Normal cases**
- User reviews the combined status of progress, procurement, BOQ overrun, and incidents.

**Abnormal cases**
- Child report access errors are surfaced as report load errors.
...
**Function description:** Detailed WBS progress report with task totals, expected progress, schedule variance, forecast, phase breakdown, acceptances, assignee performance, insights, and monthly trend.

**Function Details**

**Interface and data**
- Summary cards: Total Tasks, Done, In Progress, Assigned, New, Obsolete, Overall Progress %, Expected Progress %, Schedule Variance.
- Forecasted End Date text when calculable.
- Phase panels: phase status, start/end dates, actual/expected progress, schedule variance days, delayed tasks, all task summaries.
- Acceptance table: Phase Name, Acceptance Date, Acceptor, Cancelled flag, Cancellation Reason.
- Assignee performance table: Assignee Name, Total Tasks, Completed Tasks, Delayed Tasks, On-Time Rate.
- Monthly progress trend chart.

**Data processing**
- Loads phases with tasks, assignees, progress logs, and phase acceptances.
- Calculates weighted progress using task weight or 1 by default.
- Monthly actual progress is reconstructed from `TaskProgressLog` at month end.

**Validation and business rules**
- Obsolete tasks are excluded from active progress calculations but counted separately.
- Phase acceptance records are filtered by acceptance date when date range is provided.

**Normal cases**
- User reviews phase progress, schedule risk, and acceptance history.

**Abnormal cases**
- No phases or tasks displays empty report sections.

---

### 3.14.5. BOQ vs Actual Report Tab

**Function type:** Screen (embedded report tab)

**Function trigger:** Reports Hub -> "Định mức BOQ" tab.

**Function description:** Material control report comparing BOQ limits with actual warehouse consumption and current stock.

**Function Details**

**Interface and data**
- Summary cards: Total BOQ Item Count, Exceeding Items, Earned Exceeding Items, Saving Items, Normal Items, Total BOQ Value, Total Consumption Value, Total Variance Value.
- Material table: Material Code, Name, Unit, Unit Price, BOQ Limit, Earned BOQ Limit, Total Issued, Total Returned, Net Consumption, Stock Remaining, Pending PO Quantity, Pending MR Quantity, Usage %, Earned Usage %, Variance Value.
- Monthly BOQ consumption trend.

**Data processing**
- BOQ, issuance, return, pending PO, and pending MR quantities are normalized by conversion rate.
- Average price is project-scoped and normalized by conversion rate.
- Consumption value uses net consumption multiplied by average purchase price.
- Monthly trend uses issuance value minus return value.

**Validation and business rules**
- `NetConsumption = max(0, TotalIssued - TotalReturned)`.
- `IsExceeding = NetConsumption > BoqLimit`.
- `IsEarnedExceeding = NetConsumption > EarnedBoqLimit` when earned limit is greater than 0.
- `TotalExpectedUsage` in the DTO currently means `NetConsumption + StockRemaining`; pending PO/MR are shown separately.
- BOQ monetary value is estimated because official BOQ unit price is not stored on `BOQItem`.

**Normal cases**
- Accountant or manager reviews material usage and identifies actual consumption over BOQ.

**Abnormal cases**
- Materials without purchase price show zero estimated value.

---

### 3.14.6. Incident Report Tab

**Function type:** Screen (embedded report tab)

**Function trigger:** Reports Hub -> "Sự cố & Rework" tab.

**Function description:** Incident summary and audit table filtered by project scope and date range.

**Function Details**

**Interface and data**
- Metric cards: Total Incidents, Open Incidents, Resolved Incidents, Incidents With Rework.
- Incident table: Incident ID, Type, Description, Status, Reporter, Reviewer, Task, Phase, Damage Description, Estimated Material Loss, Estimated Delay Days, Rework Task, CreatedAt.
- Monthly incident trend: total incidents, resolved incidents, estimated loss.

**Data processing**
- Loads incidents with Reporter, Reviewer, Task, Phase, and ReworkTask.
- Resolved statuses are `Approved`, `Resolved`, `Closed`, and `Completed`.
- Date filtering uses incident `CreatedAt`.

**Validation and business rules**
- Open count is derived from records not in resolved statuses.
- Description is returned as stored; the backend report handler does not remove markdown or embedded tags.

**Normal cases**
- User audits incident count, open incidents, and related rework tasks.

**Abnormal cases**
- No incidents displays zero metrics and empty trend rows.

---

### 3.14.7. Inventory Movement Report Tab

**Function type:** Screen (embedded report tab)

**Function trigger:** Reports Hub -> "Nhập xuất tồn" movement view.

**Function description:** Shows opening balance, period movements, and closing balance by material.

**Function Details**

**Interface and data**
- Table columns: Material Code, Material Name, Unit, Opening Balance, Total Received, Total Issued, Total Returned, Total Transferred In, Total Transferred Out, Total Adjustments, Closing Balance.
- Summary: Total Materials.

**Data processing**
- Loads all inventory transactions for selected project scope, then calculates period results in memory.
- Opening balance is sum of quantity changes before `fromDate`.
- Period movement is grouped by `ReferenceType`.
- If no date range is applied, closing balance falls back to current inventory quantity.

**Validation and business rules**
- Receipt uses positive `GoodsReceipt`.
- Issuance uses negative `MaterialIssuance`.
- Return uses positive `MaterialReturn`.
- Transfer in/out use `SurplusTransferReceive` and `SurplusTransferDispatch`.
- Unknown reference types are included in total adjustments.

**Normal cases**
- User reviews inventory movement within selected period.

**Abnormal cases**
- Materials with inventory but no transactions are still included through current inventory records.

---

### 3.14.8. Inventory Ledger Report Tab

**Function type:** Screen (embedded report tab)

**Function trigger:** Reports Hub -> inventory ledger view for a specific project.

**Function description:** Displays current stock and the latest inventory transaction ledger for one project.

**Function Details**

**Interface and data**
- Summary cards: Total Material Types, Zero Stock Count.
- Current stock table: Material Code, Material Name, Unit, Current Quantity.
- Transaction ledger table: CreatedAt, Material Code, Material Name, Unit, Reference Type, Quantity Change, Balance After, Created By Name.

**Data processing**
- Requests inventory ledger report data.
- Loads current inventory by project.
- Loads latest 500 inventory transactions by project, ordered by creation time descending.
- Resolves creator names from user IDs.

**Validation and business rules**
- Ledger service is project-specific and does not accept `fromDate`/`toDate`.
- Access is checked by `IProjectAccessService`.

**Normal cases**
- User verifies current stock and recent warehouse transaction history.

**Abnormal cases**
- No transactions displays current stock with an empty ledger table.

---

### 3.14.9. Procurement Report Tab

**Function type:** Screen (embedded report tab)

**Function trigger:** Reports Hub -> "Mua sắm & Chi phí" tab.

**Function description:** Procurement cost report combining purchase orders, approved direct purchases, estimated material issuance value, and monthly procurement trend.

**Function Details**

**Interface and data**
- Cost summary cards: Total PO Cost, Total Direct Purchase Cost, Total Cost, Total Material Issuance Value, Total Procurement Savings.
- Purchase Orders table: PO ID, PO Number, Status, Supplier, Total Amount, Order Date, Expected Delivery Date.
- Direct Purchases table: Direct Purchase ID, Requester, Status, Total Amount, Purchase Date.
- Monthly procurement trend: Year, Month, PO Cost, Direct Purchase Cost, PO Count.

**Data processing**
- Requests procurement report data.
- Purchase orders are filtered by `PurchaseOrder.ProjectId`, `OrderDate`, and status not `Draft`/`Cancelled`; `Closed` is included.
- Direct purchases are filtered by `ProjectId`, `PurchaseDate`, and status `Approved`.
- Estimated material issuance value uses project-scoped average PO unit price normalized by conversion rate.

**Validation and business rules**
- `TotalCost = TotalPoCost + TotalDirectPurchaseCost`.
- `TotalProcurementSavings` currently returns 0 because no savings formula is implemented.
- Direct purchase DTO field `CreatedAt` is populated from `PurchaseDate` for compatibility with the existing response shape.

**Normal cases**
- User reviews spending by PO, direct purchase, and monthly period.

**Abnormal cases**
- No procurement transactions displays zero cost cards and empty detail tables.
