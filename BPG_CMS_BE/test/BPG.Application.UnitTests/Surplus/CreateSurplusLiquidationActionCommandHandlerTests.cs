...
namespace BPG.Application.UnitTests.Surplus
{
    public class CreateSurplusLiquidationActionCommandHandlerTests
    {
        ...
        private readonly Mock<IGenericRepository<SurplusLiquidation>> _mockLiquidationRepo;
        ...
        public CreateSurplusLiquidationActionCommandHandlerTests()
        {
            ...
            _mockLiquidationRepo = new Mock<IGenericRepository<SurplusLiquidation>>();
            ...
            _mockUow.Setup(u => u.Repository<SurplusLiquidation>()).Returns(_mockLiquidationRepo.Object);
        ...
        public async Task UTCID01_Handle_ValidLiquidation_ShouldReturnSuccess()
        {
            ...
            _mockLiquidationRepo.Verify(r => r.AddAsync(It.IsAny<SurplusLiquidation>(), It.IsAny<CancellationToken>()), Times.Once);
            _mockInventoryService.Verify(s => s.UpdateStockAsync(
                ProjectId, MaterialId, -10m, InventoryTransactionType.Liquidation, It.IsAny<long>(), EntityType.SurplusRequest, CurrentUserId, It.IsAny<CancellationToken>()), Times.Once);
        }
        ...
        public async Task UTCID04_Handle_ExceedsUncommittedQuantity_ShouldThrowBusinessException()
        {
            ...
            // Uncommitted = 20 - 5 - 10 = 5. Trying to liquidate 6.
        ...
        public async Task UTCID05_Handle_ExceedsAvailableInventory_ShouldThrowBusinessException()
        {
            ...
            // Available = 5. Trying to liquidate 10.
        ...
        [Fact]
        public async Task UTCID06_Handle_LiquidateFullQuantity_ShouldUpdateStatusToCompleted()
        {
            _mockCurrentUserService.SetupUser(CurrentUserId, RoleConstants.Accountant);
            var item = DefaultItem(quantity: 10, processedQuantity: 0);
            SetupRequestItems(item);

            await _handler.Handle(Command(qty: 10m), CancellationToken.None);
        ...
        private static CreateSurplusLiquidationActionCommand Command(decimal qty = 10m)
            => new(SurplusRequestItemId, "Người mua", qty, 1000000, null);
